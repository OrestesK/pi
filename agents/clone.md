...
Use `contact_supervisor` with `reason: "progress_update"` for the non-blocking events below. Where an event says `need_decision`, use that reason and pause instead.
...
- **Change in file ownership:** report a required expansion, conflict, release, or transfer; use `need_decision` and pause when allocation must change
...
- **Scope-changing discovery:** report evidence that changes scope, architecture, risk, proof, or the approved contract; use `need_decision` and pause when a new material choice is required
