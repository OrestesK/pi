...
- Starting a `/goal` is evidence that the user intends the task to be doable within an already-approved safe scope; it does not itself authorize sudo, mutating git, external mutation, private reads, HITL, or safety bypasses. External mutations are in scope only when the user has approved the exact target(s), action(s), expected effect, and environment boundaries. Otherwise resolve the scope before rendering or encode that path as unavailable/out of scope, then choose another safe path. Emit `GOAL_BLOCKED` only when no safe non-asking workaround exists.
...
- For nontrivial, multi-owner, behavior/API/schema, PR-sized, migration, cross-file, or otherwise risk-bearing implementation and documentation goals, include a Contract Gate: a pre-edit contract card and owner map. Do not impose it on typo-only or equally mechanical bounded goals.
...
- When the user explicitly asks to verify the best, cleanest, simplest, or most elegant approach before goal crafting, run an adversarial proposal gate first: compare credible alternatives against the complete acceptance contract, synthesize `PASS`, `FAIL`, or `INCONCLUSIVE`, and encode only the surviving approach. Do not turn the assistant's prior proposal into an approved architecture merely by placing it in a `/goal`.
- Judge design quality across the complete repeated lifecycle, including failure recovery, deployment, verification, cleanup, and final state. Prefer the best, cleanest, most cohesive and elegant complete design; minimize components and touch points only when doing so does not omit required lifecycle behavior. Reject locks, registries, controllers, compatibility layers, and fallback paths unless evidence shows the complete lifecycle needs them.
...
Use memory search when the goal depends on prior durable context. Persist only explicitly user-confirmed durable preferences or preference edits the user explicitly requested. Inferred preferences may guide research, but require user approval before editing this skill or user memory. Do not store raw transcripts or one-off details. Use session/tape/history only when it is directly relevant to the current task. Research/review artifacts should be curated and safe: do not include raw secrets or secret-like values, and if a scanner flags an artifact, require redaction/rewrite before treating it as final evidence.
...
- The goal preserves explicit user style constraints such as clean, elegant, simple, compact, or comprehensive/resource-maximal.
- A request to verify the best/cleanest/simplest approach received an adversarial proposal verdict before that approach was encoded.
