...
1. **In-scope required findings:** the actively reviewed behavior/angle
   the only partition that can block readiness or drive automatic fixes
2. **Incidental material adjacent risks:** report separately only when encountered
   do not proactively hunt them unless explicitly assigned as primary
3. **Incidental optional cleanup/polish:** report separately only when encountered
   never let it block readiness or extend a review/fix loop
...
never create findings to fill a reviewer slot or partition
