...
Report paired outcomes, ties, neither-acceptable counts, safety vetoes, confidence, and category breakdowns. Use win rates for two configs; reserve Bradley-Terry-style rankings for three or more configs or rolling leaderboards.
...
Report workflow shape, tool/subagent/todo/memory/artifact footprint, necessary vs avoidable work, process/harness issues, and an optional `process_efficiency_score` from 0 to 100. Keep this score separate from deterministic tests, hidden tests, candidate-output review, and pairwise winner verdicts.
