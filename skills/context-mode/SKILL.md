...
When an action or another tool can save large output directly, save it in a file first. Pass that path to `context_mode_ctx_execute_file` or `context_mode_ctx_index`. Do not first return the result and then send the same content again through an inline `content` argument

Use the same file-first flow for large console output, network requests, generated reports, and local data
