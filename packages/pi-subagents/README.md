...
A simple rule of thumb: use `scout` before you understand the code, `researcher` before you trust external facts, `planner` before a bigger change, `worker` to implement, `reviewer` to check, and `oracle` when the decision itself feels risky. Use fresh parallel reviewers for nontrivial review. For library/framework docs, prefer parent-fetched context7 evidence when needed; protected advisory subagents do not receive generic `mcp`. Local-code protected advisory roles receive tree-sitter, AST grep, LSP navigation/diagnostics, and safe `memory_search`/`memory_check` lookup tools when the corresponding Pi extensions are installed; the tree-sitter tools use a narrow `mcp:tree-sitter` direct-tool exception, not the generic MCP proxy.
...
Builtin agents load at the lowest priority, so a user or project agent with the same name overrides them. Protected advisory role names are still permission-sanitized after precedence is resolved: direct file mutation tools, generic `mcp`, external direct MCP tools, explicit `extensions`, custom-extension capability, and custom tool-extension paths are removed from `scout`, `reviewer`, `planner`, `researcher`, `context-builder`, `delegate`, and `oracle`. Normal extensions remain enabled only when the sanitized task tools still request a role-approved safe read-only extension-backed tool: local-code protected advisory roles may use tree-sitter symbol/search tools through the local `mcp:tree-sitter` direct-tool exception, `ast_grep_search`, `lsp_navigation`, `lsp_diagnostics`, `memory_search`, and `memory_check`, while only `researcher` and `context-builder` may use the web/code research tools. Advisory `bash` access, where present, is prompt-governed for read-only inspection because Pi has no read-only shell permission. They do not pin a provider model; they inherit your current Pi default model unless you set `subagents.agentOverrides.<name>.model`. `oracle` is an advisory reviewer that critiques direction and proposes an execution prompt without editing files. `worker` is the implementation agent for normal tasks and approved oracle handoffs.

The `researcher` builtin uses `code_search` for code/docs examples plus `web_search`, `fetch_content`, and `get_search_content` for external web research, and it can use `memory_search`/`memory_check` for durable local runbooks when `pi-memory-md` is installed. It does not receive generic `mcp`; when context7-specific library/framework evidence is required, the parent should fetch it and pass the findings into the researcher task. The web tools require [pi-web-access](https://github.com/nicobailon/pi-web-access):
...
Supported override fields are `model`, `fallbackModels`, `thinking`, `systemPromptMode`, `inheritProjectContext`, `inheritSkills`, `defaultContext`, `disabled`, `skills`, `tools`, and `systemPrompt`. Use `defaultContext: false` in builtin overrides to clear an inherited context default. Project overrides beat user overrides. For protected advisory role names, `tools` overrides are filtered to known advisory inspection/coordination tools, generic `mcp`, external direct MCP tools, explicit `extensions`, custom-extension capability, and custom tool-extension paths are stripped; normal extensions remain enabled only when the sanitized task tools still request a role-approved safe read-only extension-backed tool (tree-sitter through the local `mcp:tree-sitter` direct-tool exception, AST grep, LSP navigation/diagnostics, and `memory_search`/`memory_check` for local-code protected advisory roles; web/code research tools only for `researcher` and `context-builder`). Use `worker` or a custom non-advisory agent name when direct file mutation, generic `mcp`, external direct MCP tools, custom extensions, or custom tool-extension paths are required.
...
- **Role-agent prompting guidance**: compact contract prompts instead of long scripts, what to include in role-specific meta prompts, and research depth contracts for researchers
...
{ workflow: "builtin.live-steering-team", task: "Worker task with live reviewer steering: ..." }
