...
When the user asks for many reviewers, 8-10 review agents, different goals, or reviewers validating other reviewers, use a large review matrix rather than generic duplication. Prefer `review-matrix-reduce` for broad/high-stakes targets: first-pass reviewers by independent attack surface, then validators that examine first-pass findings for false positives, missing cross-cutting risks, and disagreements, then a reducer that dedupes/ranks blockers for parent synthesis.
