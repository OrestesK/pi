...
export function handleList(params: ManagementParams, ctx: ManagementContext): AgentToolResult<Details> {
	...
	const lines = [
		...
		"",
		"Tool access:",
		"- Tools are agent-specific, not inherited from the parent. Protected advisory roles strip MCP, mutation, direct custom-extension capability, and explicit extension tools.",
		"- Protected advisory roles keep normal extensions only when their default tool contract includes approved safe read-only extension-backed tools and the sanitized task tools still request them; use a write-capable agent for writes or a configured non-advisory agent for MCP/custom tools. Normal approval/privacy gates still apply.",
		"",
