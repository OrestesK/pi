...
function checkNoStagedFiles(cwd: string): AcceptanceRuntimeCheck {
	...
	if (result.status !== 0) {
		...
		return { id: "no-staged-files", status: "failed", message: `Unable to verify staged-file state: ${detail}` };
	}
...
const VERIFY_OUTPUT_CAPTURE_LIMIT = 12_000;

function appendBoundedOutput(current: string, chunk: Buffer, truncated: boolean): { value: string; truncated: boolean } {
	if (current.length >= VERIFY_OUTPUT_CAPTURE_LIMIT) return { value: current, truncated: true };
	const text = chunk.toString();
	const remaining = VERIFY_OUTPUT_CAPTURE_LIMIT - current.length;
	if (text.length <= remaining) return { value: current + text, truncated };
...
function runVerifyCommand(command: AcceptanceVerifyCommand, defaultCwd: string): Promise<AcceptanceVerifyResult> {
	return new Promise((resolve) => {
		...
		child.stdout.on("data", (chunk: Buffer) => {
			const next = appendBoundedOutput(stdout, chunk, stdoutTruncated);
			stdout = next.value;
			stdoutTruncated = next.truncated;
		});
		child.stderr.on("data", (chunk: Buffer) => {
			const next = appendBoundedOutput(stderr, chunk, stderrTruncated);
			stderr = next.value;
