...
export function validateAcceptanceInput(input: unknown, pathLabel = "acceptance"): string[] {
	...
	if (value.review !== undefined) {
		if (!value.review || typeof value.review !== "object" || Array.isArray(value.review)) {
			...
			if (review.required !== false) errors.push(`${pathLabel}.review.required must be false until automatic reviewer-result propagation is implemented.`);
		}
	}
...
export function resolveEffectiveAcceptance(input: {
	...
	return {
		...
		stopRules,
		finalization: input.async
			? { mode: "none", maxTurns: 0 }
			: { mode: "self-review-loop", maxTurns: explicit.maxFinalizationTurns ?? DEFAULT_FINALIZATION_MAX_TURNS },
	};
}
...
export function formatAcceptancePrompt(acceptance: ResolvedAcceptanceConfig): string {
	...
	const lines = [
		...
		...(acceptance.finalization.mode === "self-review-loop"
			...
			: ["This run will be accepted or rejected from the final acceptance report and configured runtime checks."]),
		"",
		"Criteria:",
