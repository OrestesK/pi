...
export const DEFAULT_TAPE_EXCLUDE_DIRS =
	DEFAULT_TAPE_EXCLUDE_DIRS_BY_PLATFORM[process.platform] ?? [];
...
export function toRelativeIfInside(
	...
	return isPathInside(resolvedParent, resolvedTarget)
		? path.relative(resolvedParent, resolvedTarget)
		: resolvedTarget;
}
...
export function getTapeBasePath(localPath: string, tapePath?: string): string {
	return tapePath
		? expandHomePath(tapePath)
		: path.join(localPath, DEFAULT_TAPE_DIRNAME);
}
