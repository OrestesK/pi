...
async function hasUpstreamBranch(
	...
	const upstreamResult = await gitExec(pi, cwd, [
		"rev-parse",
		"--abbrev-ref",
		"@{u}",
	]);
...
async function isAlreadyAtUpstream(
	pi: ExtensionAPI,
	cwd: string,
): Promise<boolean> {
	...
	return (
		headResult.success &&
		trackedResult.success &&
		headResult.stdout.trim() === trackedResult.stdout.trim()
	);
}

async function hasCommitsToPush(
	pi: ExtensionAPI,
	cwd: string,
): Promise<boolean> {
	if (!(await hasUpstreamBranch(pi, cwd))) return true;

	const aheadResult = await gitExec(pi, cwd, [
		"rev-list",
		"--count",
		"@{u}..HEAD",
	]);
...
export async function syncRepository(
	...
	if (fs.existsSync(localPath)) {
		...
		if (await isAlreadyAtUpstream(pi, localPath)) {
			return {
				success: true,
				message: `[${repoName}] is already latest`,
				updated: false,
			};
		}

		const pullResult = await gitExec(pi, localPath, [
			"pull",
			"--rebase",
			"--autostash",
		]);
		...
		if (!pullResult.success)
			return { success: false, message: pullResult.stdout || "Pull failed" };
	...
	const cloneResult = await gitExec(pi, parentDir, [
		"clone",
		repoUrl,
		memoryDirName,
	]);
...
export async function pushRepository(
	...
	if (!statusResult.success) {
		return {
			success: false,
			message: statusResult.stdout || "Git status failed",
		};
	}
	...
	if (hasChanges) {
		...
		const commitResult = await gitExec(pi, localPath, [
			"commit",
			"-m",
		...
		if (!commitResult.success) {
			return {
				success: false,
				message: commitResult.stdout || "Commit failed",
			};
		}
	}
	...
	if (!hasChanges && !(await hasCommitsToPush(pi, localPath))) {
		return {
			success: true,
			message: `[${repoName}] already up to date`,
			updated: false,
		};
	}
	...
	return {
		success: true,
		message: hasChanges
			? `Committed and pushed changes to [${repoName}]`
			: `[${repoName}] already up to date`,
		updated: hasChanges,
	};
}
