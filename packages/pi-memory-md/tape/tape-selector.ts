...
function entryToMessage(entry: SessionEntry): TapeMessage | null {
	switch (entry.type) {
		...
		case "model_change":
			return {
				role: "assistant",
				content: `[Model: ${entry.provider}/${entry.modelId}]`,
			};
		case "compaction":
...
export function formatEntriesAsMessages(
	entries: SessionEntry[],
): TapeMessage[] {
	return entries
		.map(entryToMessage)
		.filter((message): message is TapeMessage => message !== null);
}
...
export function matchesDefaultIgnoredPath(
	filePath: string,
	projectRoot?: string,
): boolean {
	...
	return segments.some(
		(segment) => DEFAULT_IGNORED_DIRS.has(segment) || segment.startsWith("."),
	);
}

async function getRipgrepVisibleProjectPaths(
	projectRoot: string,
...
export class MemoryFileSelector {
	...
	async selectFilesForContext(
		...
		return this.selectSmartAsync(
			limit,
			options?.memoryScan ?? DEFAULT_MEMORY_SCAN,
		);
	}
	...
	async finalizeContextFiles(filePaths: string[]): Promise<string[]> {
		...
		const selectedPathSet = new Set(
			selectedPaths.map((filePath) =>
				path.resolve(this.toAbsolutePath(filePath)),
			),
		);
		const whitelistedPaths = (
			await this.resolveListedPathsAsync(this.whitelist)
		).filter((filePath) => !selectedPathSet.has(path.resolve(filePath)));

		return [...whitelistedPaths, ...selectedPaths];
	}

	private async selectSmartAsync(
		limit: number,
		...
		while (hours <= maxHours) {
			const stats = analyzePathAccess(this.getEntriesWithinHours(hours), {
				scanHours: hours,
				resolveTrackedPath: (toolName, entryPath) =>
					this.resolveTrackedPath(toolName, entryPath),
				pathExists: (filePath) => this.pathExists(filePath),
				getLatestAnchor: (scanHours, match) =>
					this.getLatestAnchor(scanHours, match),
				getAnchorWindowEndTimestamp: (anchor, entries) =>
					this.getAnchorWindowEndTimestamp(anchor, entries),
			});
		...
		const selectedPaths = (
			await this.filterDeliverablePathsAsync(sortPathsByStats(pathStats))
		).slice(0, limit);
	...
	async buildContextFromFilesAsync(
		...
		const { fileEntries, highlightedPaths, rangeMap } =
			this.prepareContextEntries(filePaths, options);
		if (fileEntries.length === 0) return "";
		return this.renderContextFromEntriesAsync(
			fileEntries,
			highlightedPaths,
			rangeMap,
		);
	...
	private prepareContextEntries(
		...
		const existingPaths = [
			...new Set(filePaths.filter((filePath) => this.pathExists(filePath))),
		];
		...
		const rangeMap = this.getContextRangeMap(
			fileEntries,
			options?.lineRangeHours,
		);
	...
	private getContextRangeMap(
		...
		const shouldReuseLastScan =
			requestedHours === undefined &&
			lineRangeHours === this.lastSelectionScanHours;
	...
	private async renderContextFromEntriesAsync(
		...
		const { memoryEntries, projectEntries } =
			this.splitContextEntries(fileEntries);

		if (memoryEntries.length > 0) {
			const frontmatters = await Promise.all(
				memoryEntries.map((entry) =>
					this.extractFrontmatterAsync(entry.displayPath),
				),
			);
	...
	private splitContextEntries(
		...
		return {
			memoryEntries: fileEntries.filter(
				(entry) => !path.isAbsolute(entry.displayPath),
			),
			projectEntries: fileEntries.filter((entry) =>
				path.isAbsolute(entry.displayPath),
			),
		};
	}
	...
	private appendMemoryEntry(
		...
		lines.push(
			`- ${this.formatPathLabel(entry.displayPath, highlightedPaths)}`,
		);
	...
	private formatPathLabel(
		...
		return highlightedPaths.has(filePath)
			? `${filePath} [high priority]`
			: filePath;
	}

	private appendLineRanges(
		lines: string[],
		absolutePath: string,
		rangeMap: Map<string, LineRange[]>,
	): void {
	...
	private async filterDeliverablePathsAsync(
		filePaths: string[],
	): Promise<string[]> {
		const existingPaths = [
			...new Set(filePaths.filter((filePath) => this.pathExists(filePath))),
		...
		const projectPaths = existingPaths.filter(
			(filePath) =>
				path.isAbsolute(filePath) && !this.toMemoryRelativePath(filePath),
		);
		const ripgrepVisiblePaths = projectPaths.some((filePath) =>
		...
		return existingPaths.filter((filePath) => {
			...
			if (!isPathInside(this.projectRoot, this.toAbsolutePath(filePath)))
				return true;
			return (
				ripgrepVisiblePaths?.has(path.resolve(this.toAbsolutePath(filePath))) ??
				true
			);
		});
	}
	...
	private getAnchorWindowEndTimestamp(
		anchor: { timestamp: string } | null,
		entries: SessionEntry[],
	): number | null {
	...
	private async resolveListedPathsAsync(entries: string[]): Promise<string[]> {
		...
		const collectFiles = async (targetPath: string): Promise<void> => {
			try {
				...
				const childEntries = await fs.promises.readdir(targetPath, {
					withFileTypes: true,
				});
				await Promise.all(
		...
		for (const entry of entries) {
			...
			const candidatePaths = absoluteEntry
				? [absoluteEntry]
				: [
						path.resolve(this.memoryDir, entry),
						path.resolve(this.projectRoot, entry),
					];

			await Promise.all(
				candidatePaths.map((candidatePath) => collectFiles(candidatePath)),
			);
		}
	...
	private toMemoryRelativePath(filePath: string): string | null {
		const normalizedPath = toRelativeIfInside(
			this.memoryDir,
