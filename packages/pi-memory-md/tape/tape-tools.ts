...
function getAnchorSearchBounds(
	...
	if (betweenAnchors) {
		return {
			since: tapeService.findAnchorByName(betweenAnchors.start, anchorScope)
				?.timestamp,
			until: tapeService.findAnchorByName(betweenAnchors.end, anchorScope)
				?.timestamp,
			sessionId: scope === "session" ? tapeService.getSessionId() : undefined,
		};
	}
	...
	return {
		since: sinceAnchor
			? tapeService.findAnchorByName(sinceAnchor, anchorScope)?.timestamp
			: undefined,
		sessionId: scope === "session" ? tapeService.getSessionId() : undefined,
	};
}
...
function getAnchorContext(
	...
	const anchorIndex = entries.findIndex(
		(entry) => toTimestamp(entry.timestamp) >= anchorTime,
	);
...
function buildEntryQueryOptions(
	options: EntryQueryParams,
): Parameters<TapeService["query"]>[0] {
	const {
		types,
		limit,
		scope = "project",
	...
	const queryOptions: Parameters<TapeService["query"]>[0] = {
		types,
		limit,
		scope,
		anchorScope,
	...
	if (options.betweenAnchors)
		queryOptions.betweenAnchors = options.betweenAnchors;
	else if (options.betweenDates)
		queryOptions.betweenDates = options.betweenDates;
...
function getTapeUnavailableResult(): {
	...
	return {
		content: [
			{ type: "text", text: "Tape is not enabled for the current settings." },
		],
		details: {},
	};
}
...
export function registerTapeHandoff(
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			name: Type.String({
				description:
					"Anchor name (e.g., 'task/begin', 'task/complete', 'handoff')",
			}),
			summary: Type.Optional(
				Type.String({
					description: "Brief intent summary of current task (under 18 words)",
				}),
			),
			purpose: Type.Optional(
				Type.String({ description: "1-2 word label for the anchor's purpose" }),
			),
		}),
		...
		async execute(_toolCallId, params) {
			...
			const keywordHandoffMatch =
				handoffMatch?.trigger === "keyword" ? handoffMatch : null;
			const matchedKeywordHandoff =
				keywordHandoffMatch?.instruction.anchorName === name;
			const finalTrigger =
				handoffMatch?.trigger === "manual"
					? "manual"
					: matchedKeywordHandoff
						? "keyword"
						: "direct";
			const finalKeywords =
				finalTrigger === "keyword"
					? normalizeKeywords(keywordHandoffMatch?.instruction.matched)
					: undefined;
			...
			const mergedMeta = normalizeHandoffMeta(
				summary,
				purpose,
				finalTrigger,
				finalKeywords,
			...
			return {
				...
				details: {
					...
					hasHandoffMatch: handoffMatch !== null,
					matchedKeywordHandoff:
						handoffMatch?.trigger === "keyword" ? matchedKeywordHandoff : false,
				},
			};
		},
		...
		renderResult(result, state: RenderState, theme) {
			if (state.isPartial)
				return renderText(theme.fg("warning", "Creating anchor..."));
			...
			return renderText(
				`${theme.fg("success", "Anchor created:")}\n${theme.fg("toolOutput", text)}`,
			);
		},
	});
}
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			contextLines: Type.Optional(
				Type.Integer({
					description:
						"Number of context lines before/after each anchor (default: 1)",
					minimum: 0,
					maximum: 5,
				}),
			),
		...
		async execute(_toolCallId, params) {
			...
			const { limit = 20, contextLines = 1 } = params as {
				limit?: number;
				contextLines?: number;
			};
			...
			const projectEntries = tapeService.query({
				scope: "project",
				anchorScope: "project",
			});

			const anchorsWithContext = anchors.map((anchor) => {
				const [beforeContext, afterContext] = getAnchorContext(
					projectEntries,
					anchor.timestamp,
					contextLines,
				);
			...
			if (anchorsWithContext.length > 0) {
				summary =
					...
					anchorsWithContext
						.map((anchor) => {
							const metaStr =
								Object.keys(anchor.meta).length > 0
			...
			return {
				...
				details: {
					anchors: anchorsWithContext,
					count: anchorsWithContext.length,
				},
			};
		},
		...
		renderCall(args, theme) {
			...
			if (args.contextLines)
				text += ` ${theme.fg("muted", `context=${args.contextLines}`)}`;
			return renderText(text);
		},

		renderResult(result, state: RenderState, theme) {
			if (state.isPartial)
				return renderText(theme.fg("warning", "Listing anchors..."));
			...
			if (!state.expanded)
				return renderText(
					theme.fg("success", `${details?.count ?? 0} anchor(s)`),
				);
			return renderText(
				theme.fg(
					"toolOutput",
...
export function registerTapeAnchorDelete(
	pi: ExtensionAPI,
	getTapeService: TapeServiceGetter,
): void {
	pi.registerTool({
		...
		parameters: Type.Object({
			id: Type.String({
				description: "Anchor id to delete (use tape_list to list ids)",
			}),
		}),
		...
		renderCall(args, theme) {
			return renderText(
				theme.fg("toolTitle", theme.bold("tape_delete ")) +
		...
		renderResult(result, state: RenderState, theme) {
			if (state.isPartial)
				return renderText(theme.fg("warning", "Deleting anchor..."));
			...
			return renderText(
				`${theme.fg("success", "Anchor deleted:")}\n${theme.fg("toolOutput", text)}`,
			);
		},
	});
}
...
export function registerTapeInfo(
	...
	pi.registerTool({
		...
		renderResult(result, state: RenderState, theme) {
			const details = result.details as
				| { totalEntries?: number; anchorCount?: number }
				| undefined;
...
const SearchKindsUnion = Type.Union([
	Type.Literal("entry"),
	Type.Literal("anchor"),
	Type.Literal("all"),
]);
const QueryScopeUnion = Type.Union([
	Type.Literal("session"),
	Type.Literal("project"),
]);
const AnchorScopeUnion = Type.Union([
...
export function registerTapeSearch(
	...
	pi.registerTool({
		...
		label: "Tape Search",
		description:
			"Search tape entries and anchors by type, content, or time range",
		parameters: Type.Object({
			kinds: Type.Optional(
				Type.Array(SearchKindsUnion, {
					description:
						"What to search: 'entry' (session entries), 'anchor' (anchors), 'all' (default: all)",
				}),
			),
			types: Type.Optional(
				Type.Array(EntryTypeUnion, {
					description: "Filter entries by type (only for entries search)",
				}),
			),
			limit: Type.Optional(
				Type.Integer({
			...
			sinceAnchor: Type.Optional(
				Type.String({ description: "Anchor name to search from" }),
			),
			lastAnchor: Type.Optional(
				Type.Boolean({ description: "Search from last anchor" }),
			),
			betweenAnchors: Type.Optional(
				Type.Object(
					{ start: Type.String(), end: Type.String() },
					{ description: "Between two anchors" },
				),
			),
			betweenDates: Type.Optional(
				Type.Object(
					{ start: Type.String(), end: Type.String() },
					{ description: "Between dates (ISO)" },
				),
			),
			scope: Type.Optional(
				Type.Unsafe({
					...QueryScopeUnion,
					description: "Entry scope: 'session' or 'project' (default: project)",
				}),
			),
			anchorScope: Type.Optional(
				Type.Unsafe({
					...AnchorScopeUnion,
					description:
						"Anchor resolution: 'current-session' (default) or 'project'",
				}),
			),
			query: Type.Optional(
				Type.String({ description: "Text search in entry/anchor content" }),
			),
			anchorName: Type.Optional(
				Type.String({ description: "Filter anchors by name substring" }),
			),
			anchorType: Type.Optional(
				Type.String({
					description: "Filter anchors by exact type, e.g. 'handoff'",
				}),
			),
			anchorSummary: Type.Optional(
				Type.String({ description: "Filter anchors by summary substring" }),
			),
			anchorPurpose: Type.Optional(
				Type.String({ description: "Filter anchors by purpose substring" }),
			),
			anchorKeywords: Type.Optional(
				Type.Array(Type.String(), {
					description: "Filter anchors that contain all given keywords",
				}),
			),
		}),
		...
		async execute(_toolCallId, params) {
			...
			if (kinds.includes("anchor") || kinds.includes("all")) {
				...
				if (anchorCount > 0) {
					...
					for (const anchor of anchors.slice(-5)) {
						const metaStr = anchor.meta
							? ` ${JSON.stringify(anchor.meta)}`
							: "";
						lines.push(
							`  ${anchor.name} [${anchor.type}] (${toLocaleDateTime(anchor.timestamp)})${metaStr}`,
						);
					}
			...
			const header =
				parts.length > 0 ? `Found ${parts.join(", ")}` : "No results";
		...
		renderCall(args, theme) {
			...
			if (args.kinds?.length)
				parts.push(theme.fg("muted", args.kinds.join(",")));
			if (args.sinceAnchor)
				parts.push(theme.fg("accent", `@${args.sinceAnchor}`));
			if (args.query) parts.push(theme.fg("accent", `"${args.query}"`));
			if (args.anchorSummary)
				parts.push(theme.fg("accent", `summary:${args.anchorSummary}`));
			if (args.anchorPurpose)
				parts.push(theme.fg("accent", `purpose:${args.anchorPurpose}`));
			if (args.anchorKeywords?.length)
				parts.push(
					theme.fg("accent", `keywords:${args.anchorKeywords.join(",")}`),
				);
...
export function registerTapeRead(
	...
	pi.registerTool({
		...
		label: "Tape Read",
		description:
			"Read tape entries from pi session with anchor, date, or query filters.",
		parameters: Type.Object({
			afterAnchor: Type.Optional(
				Type.String({ description: "Read entries after this anchor" }),
			),
			lastAnchor: Type.Optional(
				Type.Boolean({ description: "Read entries after last anchor" }),
			),
			betweenAnchors: Type.Optional(
				Type.Object(
					{ start: Type.String(), end: Type.String() },
			...
			types: Type.Optional(
				Type.Array(EntryTypeUnion, { description: "Filter entries by type" }),
			),
			scope: Type.Optional(
				Type.Unsafe({
					...QueryScopeUnion,
			...
			anchorScope: Type.Optional(
				Type.Unsafe({
					...AnchorScopeUnion,
					description: "Anchor resolution: 'current-session' or 'project'",
				}),
			),
			limit: Type.Optional(
				Type.Integer({
		...
		async execute(_toolCallId, params) {
			...
			return {
				content: [
					{
						type: "text",
						text: `Retrieved ${entries.length} entries:\n\n${formatted || "(no entries)"}`,
					},
				],
		...
		renderCall(args, theme) {
			...
			if (args.afterAnchor)
				parts.push(theme.fg("muted", `after=${args.afterAnchor}`));
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			archive: Type.Optional(
				Type.Boolean({
					description: "Archive old tape first (not implemented)",
				}),
			),
		}),
		...
		renderCall(args, theme) {
			return renderText(
				theme.fg("toolTitle", theme.bold("tape_reset")) +
		...
		renderResult(result, state: RenderState, theme) {
			if (state.isPartial)
				return renderText(theme.fg("warning", "Resetting..."));
			return renderText(
				theme.fg(
					"success",
					(result.content[0] as { text?: string })?.text ?? "",
