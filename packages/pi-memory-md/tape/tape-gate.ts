...
export function resolveTapeGate(
	cwd: string,
	tape?: TapeConfig,
): TapeGateResult {
...
export function normalizeTapeKeywords(
	config?: TapeKeywordConfig,
): TapeKeywordConfig {
	return {
...
export function detectKeywordHandoff(
	prompt: string,
	config?: TapeKeywordConfig,
): KeywordHandoffInstruction | null {
	const normalizedPrompt = prompt.trim();
	if (
		normalizedPrompt.length < MIN_KEYWORD_PROMPT_LENGTH ||
		normalizedPrompt.length > MAX_KEYWORD_PROMPT_LENGTH
	) {
		return null;
	}

	const keywords = [
		...normalizeKeywordList(config?.global),
		...normalizeKeywordList(config?.project),
	];
	const matched = [
		...new Set(
			keywords.filter((keyword) => matchesKeyword(normalizedPrompt, keyword)),
		),
	].sort(
...
export function buildKeywordHandoffMessage(
	prompt: string,
	config?: TapeKeywordConfig,
): string | null {
