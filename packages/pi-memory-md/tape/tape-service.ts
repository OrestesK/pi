...
function stripAnchorLabel(
	...
	const baseLabel = label
		.split(getAnchorLabelSeparator(labelPrefix), 1)[0]
		.trim();
	return baseLabel || undefined;
}

function mergeAnchorLabel(
	labelPrefix: string,
...
export class TapeService {
	...
	private entryCache = new Map<
		"session" | "project",
		{ signature: string; entries: SessionEntry[] }
	>();
	...
	configureSessionTree(
		sm: TapeSessionManager,
		prefix?: string,
		labelWriter?: TapeLabelSetter,
	): void {
		const nextPrefix =
			prefix && prefix.trim().length > 0 ? prefix : DEFAULT_ANCHOR_LABEL_PREFIX;
	...
	recordSessionStart(
		...
		const hasExistingEntries =
			(this.sessionManager?.getEntries().length ?? 0) > 0;
		const anchorName =
			reason === "new" || (reason === "startup" && !hasExistingEntries)
				? "session/new"
				: "session/resume";
	...
	createAnchor(
		...
		const sessionEntryId =
			this.sessionManager?.getLeafId() ?? crypto.randomUUID();
	...
	query(options: TapeQueryOptions & { since?: string }): SessionEntry[] {
		...
		if (endTime) {
			...
			entries = entries.filter(
				(entry) => toTimestamp(entry.timestamp) <= endTimestamp,
			);
		}
	...
	private getLabelMaps(): {
		labelsById: Map<string, string>;
		labelTimestampsById: Map<string, string>;
	} | null {
		if (
			!this.sessionManager?.labelsById ||
			!this.sessionManager.labelTimestampsById
		)
			return null;
		return {
	...
	private syncTreeLabel(sessionEntryId: string): void {
		...
		const anchors = this.anchorStore.query({
			sessionEntryId,
			sessionId: this.sessionId,
		});
	...
	private syncSessionTreeLabels(): void {
		...
		for (const anchor of this.anchorStore.query({
			sessionId: this.sessionId,
		})) {
	...
	syncAnchorTreeLabel(anchorId: string): void {
		const anchor =
			this.anchorStore.query({ id: anchorId, returnMode: "first" })[0] ?? null;
	...
	findAnchorByName(
		name: string,
		anchorScope: "current-session" | "project" = "current-session",
	): TapeAnchor | null {
	...
	getInfo(): {
		...
		const sessionAnchors = this.anchorStore.query({
			sessionId: this.sessionId,
		});
		...
		if (lastAnchor) {
			entriesSinceLastAnchor = getEntriesAfterTimestamp(
				sessionEntries,
