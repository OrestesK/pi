...
function parseAnchorLine(line: string): TapeAnchor | null {
	try {
		...
		return {
			id:
				rawEntry.id ??
				`${rawEntry.sessionEntryId}:${rawEntry.timestamp}:${rawEntry.name}`,
			name: rawEntry.name,
			type: rawEntry.type,
...
export class AnchorStore {
	...
	private addToMemoryIndex(entry: TapeAnchor): void {
		...
		const sessionEntryKey = this.getSessionEntryKey(
			entry.sessionEntryId,
			entry.sessionId,
		);
		const bySessionEntry =
			this.anchorsBySessionEntry.get(sessionEntryKey) ?? [];
	...
	private queryFile(options: QueryOptions): TapeAnchor[] {
		...
		if (error) {
			console.error(
				`[AnchorStore] Failed to read index file: ${error.message}`,
			);
			return [];
		}
		...
		const normalizedName =
			nameCaseInsensitive && name ? name.toLowerCase() : undefined;
		...
		for (const entry of entries) {
			...
			if (
				...
				continue;
			if (name !== undefined && !nameCaseInsensitive && entry.name !== name)
				continue;
			if (sessionId !== undefined && entry.sessionId !== sessionId) continue;
			if (
				sessionEntryId !== undefined &&
				entry.sessionEntryId !== sessionEntryId
			)
				continue;
			results.push(entry);
		}
	...
	query(options: QueryOptions): TapeAnchor[] {
		...
		const fileResults = this.queryFile({
			id,
			name,
			nameCaseInsensitive,
			sessionId,
	...
	private queryMemory(options: QueryOptions): TapeAnchor[] {
		const { id, name, nameCaseInsensitive, sessionId, sessionEntryId } =
			options;
		const normalizedName =
			nameCaseInsensitive && name ? name.toLowerCase() : undefined;
		const results: TapeAnchor[] = [];
		...
		for (const anchor of this.allAnchors) {
			...
			if (
				normalizedName !== undefined &&
				anchor.name.toLowerCase() !== normalizedName
			)
				continue;
			if (name !== undefined && !nameCaseInsensitive && anchor.name !== name)
				continue;
			if (sessionId !== undefined && anchor.sessionId !== sessionId) continue;
			if (
				sessionEntryId !== undefined &&
				anchor.sessionEntryId !== sessionEntryId
			)
				continue;
			results.push(anchor);
		}
	...
	search(options: {
		...
		let anchors = sessionId
			? [...(this.anchorsBySession.get(sessionId) ?? [])]
			: [...this.allAnchors];
		...
		if (sinceTime !== null) {
			anchors = anchors.filter(
				(anchor) => toTimestamp(anchor.timestamp) >= sinceTime,
			);
		}

		if (untilTime !== null) {
			anchors = anchors.filter(
				(anchor) => toTimestamp(anchor.timestamp) <= untilTime,
			);
		}
		...
		if (needle) {
			anchors = anchors.filter(
				(anchor) =>
					...
					(anchor.meta &&
						JSON.stringify(anchor.meta).toLowerCase().includes(needle)),
			);
		}
		...
		if (name) {
			...
			anchors = anchors.filter((anchor) =>
				anchor.name.toLowerCase().includes(normalizedName),
			);
		}
		...
		if (summary) {
			...
			anchors = anchors.filter((anchor) =>
				anchor.meta?.summary?.toLowerCase().includes(normalizedSummary),
			);
		}
		...
		if (purpose) {
			...
			anchors = anchors.filter((anchor) =>
				anchor.meta?.purpose?.toLowerCase().includes(normalizedPurpose),
			);
		}
		...
		if (keywords?.length) {
			...
			anchors = anchors.filter((anchor) => {
				const anchorKeywords =
					anchor.meta?.keywords?.map((keyword) => keyword.toLowerCase()) ?? [];
				return normalizedKeywords.every((keyword) =>
					anchorKeywords.includes(keyword),
				);
			});
		}
