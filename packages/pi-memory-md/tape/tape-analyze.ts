	...
	for (const entry of entries) {
		...
		for (const block of messageEntry.message.content) {
			...
			const boost = anchorWindows.reduce(
				(total, anchorWindow) =>
					total + getAnchorBoost(accessTime, anchorWindow),
				0,
			);
...
export function sortPathsByStats(
	pathStats: Map<string, MemoryPathStats>,
): string[] {
	return Array.from(pathStats.entries())
		.sort(([, left], [, right]) => {
			...
			return (
				rightFinalScore - leftFinalScore ||
				right.score - left.score ||
				right.lastAccess - left.lastAccess
			);
		})
...
export function analyzeRecentLineRanges(
	...
	const targetPaths = new Set(
		options.targetPaths.map((filePath) =>
			path.resolve(options.toAbsolutePath(filePath)),
		),
	);
	...
	for (const entry of entries) {
		...
		if (
			messageEntry.message.role === "assistant" &&
			...
			for (const block of messageEntry.message.content) {
				...
				const trackedPath = entryPath
					? options.resolveTrackedPath(toolName, entryPath)
					: null;
		...
		if (
			messageEntry.message.role !== "toolResult" ||
			messageEntry.message.toolName !== "edit"
		) {
			continue;
		}
...
function extractEditRanges(
	details: { firstChangedLine?: number; diff?: string } | undefined,
): LineRange[] {
...
function createAnchorWindows(
	entries: SessionEntry[],
	options: AnalyzePathAccessOptions,
): AnchorWindow[] {
	const recentHandoffAnchor = options.getLatestAnchor(
		options.scanHours,
		(anchor) => anchor.type === "handoff",
	);
	...
	return [
		{
			...
			windowEnd: options.getAnchorWindowEndTimestamp(
				recentHandoffAnchor,
				entries,
			),
			boost: HANDOFF_BOOST,
...
function recordToolAccess(
	stats: MemoryPathStats,
	toolName: SupportedPathToolName,
): void {
	switch (toolName) {
		// case "memory_read":
...
function getRecencyBonus(lastAccess: number): number {
	const hoursSinceLastAccess = Math.max(
		0,
		(Date.now() - lastAccess) / (1000 * 60 * 60),
	);
