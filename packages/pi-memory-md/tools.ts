...
function renderCollapsed(
	...
	return renderText(
		theme.fg("success", summary) +
			buildExpandHint(fullText.split("\n").length, theme),
	);
}
...
function renderMemoryResult(
	...
	const details = result.details as
		...
		| undefined;
	if (details?.error)
		return renderText(theme.fg("error", getResultText(result) || "Error"));

	const description =
		defaults?.description || details?.frontmatter?.description || "Memory file";
	...
	if (!options.expanded) {
		...
		return renderText(
			summary + buildExpandHint(text.split("\n").length + 2, theme),
		);
	}
...
function renderSyncResult(
	...
	const details = result.details as
		| { success?: boolean; initialized?: boolean; timeout?: boolean }
		| undefined;
	if (details?.initialized === false)
		return renderText(theme.fg("muted", "Not initialized"));
	if (details?.timeout)
		return renderText(theme.fg("error", getResultText(result)));
	...
	if (!options.expanded) {
		...
		if (details?.success === false) {
			return renderText(
				theme.fg("error", lines[0] || "Operation failed") +
					buildExpandHint(lines.length, theme),
			);
		}
...
function renderCountResult(
	...
	if (!options.expanded)
		return renderText(
			theme.fg("success", `${details?.count ?? 0} ${label}`) +
				buildExpandHint(text.split("\n").length, theme),
		);
	return renderText(theme.fg("toolOutput", text));
}
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			if (!settings.localPath) {
				return {
					content: [
						{ type: "text", text: "Memory localPath is not configured." },
					],
					details: { success: false, error: "localPath not configured" },
				};
			}
				...
				if (memoryRepo.gitRoot !== memoryRepo.cwd) {
					return {
						content: [
							{
								type: "text",
								text: "Memory is initialized but not git-backed. Configure repoUrl before using pull or push sync.",
							},
						],
						details: {
							initialized: true,
							gitBacked: false,
							projectMemory: hasProjectMemory,
				...
				if (!result.success) {
					return {
						content: [
							{
								type: "text",
								text: `Git status failed: ${result.stdout || "Unknown error"}`,
							},
						],
			...
			if (action === "push") {
				...
				return {
					...
					details: {
						...
						committed: result.updated ?? false,
					},
				};
			}
		...
		renderCall: (args, theme) =>
			new Text(buildToolCallText("memory_sync", args, theme), 0, 0),
		renderResult: (result, options, theme) =>
			renderSyncResult(result, options, theme),
	});
}
...
export function registerMemoryWrite(
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			path: Type.String({
				description:
					"Project memory relative path (e.g., 'core/project/architecture.md')",
			}),
		...
		renderCall: (args, theme) =>
			new Text(buildToolCallText("memory_write", args, theme), 0, 0),
		renderResult: (result, options, theme) => {
			const details = result.details as {
				frontmatter?: { description?: string; tags?: string[] };
			};
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			directory: Type.Optional(
				Type.String({
					description: "Project subdirectory (e.g., 'core/project')",
				}),
			),
		}),
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			function toProjectRelativePaths(files: string[]): string[] {
				return files.map((filePath) =>
					path.relative(projectMemoryDir, filePath),
				);
			}
			...
			if (directory) {
				...
				if (!listDir || hasSymlinkInPath(projectMemoryDir, listDir)) {
					return {
						content: [
							{ type: "text", text: `Invalid memory directory: ${directory}` },
						],
		...
		renderCall: (args, theme) =>
			new Text(buildToolCallText("memory_list", args, theme), 0, 0),
		renderResult: (result, options, theme) =>
			renderCountResult(result, options, theme, "memory files"),
	});
}
...
export function registerMemorySearch(
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			grep: Type.Optional(
				Type.String({
					description: "Custom grep pattern (use with tool: 'grep')",
				}),
			),
			rg: Type.Optional(
				Type.String({
					description: "Custom ripgrep pattern (use with tool: 'rg')",
				}),
			),
		}),
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const globalMemoryDir = getGlobalMemoryDir(settings);
			const searchRoots = [
				...(globalMemoryDir && globalMemoryDir !== memoryDir
					? [{ label: "Shared global", memoryDir: globalMemoryDir }]
					: []),
				{ label: "Project", memoryDir },
			];
			const searchDirs = searchRoots.filter((root) =>
				fs.existsSync(root.memoryDir),
			);
			...
			if (searchDirs.length === 0) {
				return {
					content: [
						{
							type: "text",
							text: `Memory directories not found: ${searchRoots.map((root) => root.memoryDir).join(", ")}`,
						},
					],
			...
			if (!query && !grep && !rg) {
				return {
					content: [
						{
							type: "text",
							text: "Provide query, grep, or rg to search memory files.",
						},
					],
					details: { files: [], count: 0 },
				};
			...
			async function runTool(tool: string, args: string[]): Promise<string[]> {
				...
				const { stdout } = await pi
					.exec(tool, args, { signal: controller.signal })
					.catch(() => ({ stdout: "" }));
			...
			for (const { label, memoryDir: searchDir } of searchDirs) {
				...
				if (grep) {
					...
					if (grepResults.length > 0) {
						sections.push(
							"",
							`## ${label} custom grep: ${grep}`,
							...grepResults.slice(0, 50),
						);
					}
				}

				if (rg) {
					const rgResults = await runTool("rg", [
						"-t",
						"md",
						"-m",
						String(MAX_SEARCH_RESULTS),
					...
					if (rgResults.length > 0) {
						sections.push(
							"",
							`## ${label} custom ripgrep: ${rg}`,
							...rgResults.slice(0, 50),
						);
					}
				}
			}
			...
			if (sections.length === 0) {
				return {
					content: [
						{ type: "text", text: `No results found for "${searchLabel}".` },
					],
					details: { files: [], count: 0 },
				};
			}
...
export function registerMemoryCheck(
	...
	pi.registerTool({
		...
		async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
			...
			const projectMemoryExists = fs.existsSync(projectMemoryDir);
			const globalMemoryExists =
				!!globalMemoryDir && fs.existsSync(globalMemoryDir);

			if (!projectMemoryExists && !globalMemoryExists) {
				const missingGlobalMessage = globalMemoryDir
				...
				return {
					...
					details: {
						...
						globalMemoryDir,
						globalMemoryMissing: !!globalMemoryDir,
					},
				};
			}
			...
			function renderTree(memoryDir: string): string {
				try {
					return execSync(
						`tree -L 3 -I "node_modules" "${memoryDir}" 2>/dev/null`,
						{ encoding: "utf-8" },
					);
				} catch {
					try {
						return execSync(
							`find "${memoryDir}" -type d -not -path "*/node_modules/*" 2>/dev/null | head -20`,
							{
								encoding: "utf-8",
							},
						);
		...
		renderCall: (_args, theme) =>
			new Text(buildToolCallText("memory_check", {}, theme), 0, 0),
		renderResult: (result, options, theme) => {
			if (options.isPartial)
				return renderText(theme.fg("warning", "Checking..."));
			const details = result.details as {
				exists?: boolean;
				fileCount?: number;
