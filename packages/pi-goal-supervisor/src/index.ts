...
Use GOAL_BLOCKED only for an actual automatic command/tool/runtime blocker; a missing required tool, resource, credential, auth, access, or service; or a required unauthorized protected action with no safe alternative. For the protected-action class, write exactly: GOAL_BLOCKED: required protected action not authorized; no safe alternative: action=<specific action>; effect=<required effect>; evidence=<evidence>. For another allowed class, write: GOAL_BLOCKED: <specific blocker and evidence that no safe non-asking next step exists>.
