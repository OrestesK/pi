...
test("store returns exact line windows and cited search matches", async () => {
	...
	const window = await store.getLineWindow(stored.sourceId, { lineStart: 2, lineLimit: 2 });
	...
	assert.deepEqual(
		matches.map((match) => ({ sourceId: match.sourceId, lineNumber: match.lineNumber, line: match.line })),
		[
...
test("store searches recent sources first when sourceId is omitted", async () => {
	...
	const broadMatches = await store.search("needle", { limit: 1, contextLines: 0 });
	assert.deepEqual(
		broadMatches.map((match) => ({ sourceId: match.sourceId, line: match.line })),
		[{ sourceId: recentSource.sourceId, line: "needle from recent source" }],
	);

	const restrictedMatches = await store.search("needle", { sourceId: oldSource.sourceId, limit: 1, contextLines: 0 });
	assert.deepEqual(
		restrictedMatches.map((match) => ({ sourceId: match.sourceId, line: match.line })),
...
test("broad store search builds an FTS candidate index without changing newest-first matches", async () => {
	...
	assert.equal((await stat(join(dir, "search-index.sqlite"))).mode & 0o777, 0o600);
});
...
test("broad store searches fall back to linear scan when SQLite indexing is unavailable", async () => {
	...
	const store = new ToolResultStore(dir, { searchIndexFactory: async () => undefined });
	const stored = await store.storeSource({
		toolName: "bash",
...
test("broad FTS candidate search escapes query syntax before exact line scanning", async () => {
	...
	const matches = await store.search("alpha \"beta\"", { limit: 1, contextLines: 0 });
	...
	try {
		...
		const columns = db.prepare("PRAGMA table_info(indexed_sources)").all().map((row) => row.name);
		assert.equal(version, 2);
		assert.deepEqual(columns, ["id", "source_id", "sha256"]);
	} finally {
		db.close();
	}
	await assert.rejects(() => stat(join(dir, "search-index.sqlite-wal")), { code: "ENOENT" });
	await assert.rejects(() => stat(join(dir, "search-index.sqlite-shm")), { code: "ENOENT" });
});
...
test("broad FTS candidate search preserves exact line context and first-hit columns", async () => {
	...
	assert.deepEqual(
		matches.map((match) => ({ sourceId: match.sourceId, lineNumber: match.lineNumber, contextStartLine: match.contextStartLine, contextEndLine: match.contextEndLine, matchStartColumn: match.matchStartColumn, matchEndColumn: match.matchEndColumn, context: match.context })),
		[{ sourceId: stored.sourceId, lineNumber: 2, contextStartLine: 1, contextEndLine: 3, matchStartColumn: 0, matchEndColumn: 6, context: "before\nneedle then needle again\nafter\n" }],
	);
});
...
test("broad search falls back to linear scan when the FTS sidecar is unreadable", async () => {
	...
	await writeFile(join(dir, "search-index.sqlite"), "not a sqlite database", { mode: 0o600 });

	const matches = await store.search("needle", { limit: 1, contextLines: 0 });

	assert.deepEqual(
		matches.map((match) => ({ sourceId: match.sourceId, line: match.line })),
		[{ sourceId: stored.sourceId, line: "needle survives corrupt sqlite sidecar" }],
	);
});
...
test("store skips malformed index rows while preserving valid sources", async () => {
	...
	assert.deepEqual((await store.listSources()).map((source) => source.sourceId), [stored.sourceId]);
	const stats = await store.getStats();
	assert.equal(stats.invalidIndexLineCount, 2);
	assert.equal(stats.indexLineCount, 3);
	assert.equal((await store.readSource(stored.sourceId)).text, "needle survives malformed index rows\n");
	const matches = await store.search("needle", { limit: 1, contextLines: 0 });
	assert.deepEqual(
		matches.map((match) => ({ sourceId: match.sourceId, line: match.line })),
		[{ sourceId: stored.sourceId, line: "needle survives malformed index rows" }],
	);
});
...
test("store rejects poisoned index paths outside managed sidecar directories", async () => {
	...
	await writeFile(
		join(dir, "index.jsonl"),
		[
			JSON.stringify({ ...stored, sourceId: "tr_poison_text", textPath: outsideSecret }),
			JSON.stringify({ ...stored, sourceId: "tr_poison_details", originalDetailsPath: outsideSecret, originalDetailsByteCount: 29, originalDetailsSha256: "fake" }),
			JSON.stringify(stored),
		].join("\n") + "\n",
		{ encoding: "utf8", mode: 0o600 },
	);
	...
	const stats = await store.getStats();
	assert.equal(stats.invalidIndexLineCount, 2);
	await assert.rejects(() => store.getLineWindow("tr_poison_text"), /Unknown tool-result source/);
	await assert.rejects(() => store.exportOriginalDetails("tr_poison_details"), /Unknown tool-result source/);
	const matches = await store.search("POISONED_INDEX_SECRET", { limit: 1, contextLines: 0 });
	assert.deepEqual(matches, []);
});

test("store previews retention candidates without deleting stored sources", async () => {
	const { store } = await makeStore();
	const originalDetailsText = JSON.stringify({ matches: Array.from({ length: 12 }, (_unused, index) => ({ tool: `old_tool_${index}` })) });
	const oldSource = await store.storeSource({
		toolName: "bash",
		text: "old source\n",
		captureStatus: "event.content",
	...
	assert.equal(stats.totalOriginalDetailsBytes, Buffer.byteLength(originalDetailsText, "utf8"));
	assert.equal(stats.totalStoredBytes, stats.totalBytes + stats.totalOriginalDetailsBytes);
	...
	assert.equal(preview.candidateDetailsBytes, Buffer.byteLength(originalDetailsText, "utf8"));
	assert.equal(preview.candidateStoredBytes, preview.candidateBytes + preview.candidateDetailsBytes);
	assert.deepEqual(preview.candidates.map((candidate) => candidate.sourceId), [oldSource.sourceId]);
	assert.equal(preview.keptSourceIds.includes(recentSource.sourceId), true);
	assert.equal((await store.readSource(oldSource.sourceId)).text, "old source\n");
});
...
test("broad search rebuilds FTS sidecar when FTS rows are missing", async () => {
	...
	assert.equal((await store.search("needle", { limit: 1, contextLines: 0 })).length, 1);

	const sqlite = await import("node:sqlite");
	const db = new sqlite.DatabaseSync(join(dir, "search-index.sqlite"));
	try {
		db.exec("DROP TABLE sources_fts");
		db.exec("CREATE VIRTUAL TABLE sources_fts USING fts5(text, content='', tokenize='trigram')");
	} finally {
		db.close();
	}
...
test("store exports full sources and exact line ranges without returning raw content", async () => {
	const { store, dir } = await makeStore();
	const stored = await store.storeSource({
		toolName: "bash",
		text: "one\ntwo\nthree\nfour\n",
	...
	const fullExport = await store.exportSource(stored.sourceId);
	assert.equal(await readFile(fullExport.filePath, "utf8"), "one\ntwo\nthree\nfour\n");
	assert.equal(fullExport.lineCount, 4);

	const rangedExport = await store.exportSource(stored.sourceId, { lineStart: 2, lineLimit: 2, filePath: "range.txt" });
	assert.equal(rangedExport.filePath, join(dir, "exports", "range.txt"));
	assert.equal(await readFile(rangedExport.filePath, "utf8"), "two\nthree\n");
	assert.equal(rangedExport.lineCount, 2);

	const longFilePath = `${"P".repeat(5_000)}.txt`;
	await assert.rejects(() => store.exportSource(stored.sourceId, { filePath: longFilePath }), /Invalid filePath/);
});
...
test("store writes sidecar files with owner-only permissions", async () => {
	...
	const exported = await store.exportSource(stored.sourceId);
	const exportedDetails = await store.exportOriginalDetails(stored.sourceId);
	...
	assert.equal((await stat(exported.filePath)).mode & 0o777, 0o600);
	assert.equal((await stat(exportedDetails.filePath)).mode & 0o777, 0o600);
});
...
test("store rolls back just-written sidecar files when index append fails", async () => {
	...
	assert.deepEqual(await readdir(join(dir, "sources")), []);
	assert.deepEqual(await readdir(join(dir, "details")), []);
});
