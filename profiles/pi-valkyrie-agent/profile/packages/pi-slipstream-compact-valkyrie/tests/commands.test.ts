...
describe("commands", () => {
	...
	it("allows only the exact normalized Valkyrie artifact root outside cwd", async () => {
		assert.equal(
			await resolveArtifactRoot(
				"/repo/project",
				"/logs/ok-pi-agent/compactions/.",
			),
			"/logs/ok-pi-agent/compactions",
		);
		for (const path of [
			"/logs/ok-pi-agent",
			"/logs/ok-pi-agent/other",
			"/logs/ok-pi-agent/compactions-extra",
			"/logs/ok-pi-agent/compactions/../other",
		]) {
			await assert.rejects(
				() => resolveArtifactRoot("/repo/project", path),
				/artifactRoot must resolve inside/,
			);
		}
	});
