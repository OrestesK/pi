...
export function loadSettings(_cwd = process.cwd()): SlipstreamConfig {
	const agentDir =
		process.env.PI_CODING_AGENT_DIR ?? join(homedir(), ".pi", "agent");
	const globalSettings = readJsonObject(join(agentDir, "settings.json"));
	const config = getNestedConfig(globalSettings);
	const reserveTokens = compactionReserveTokens(globalSettings);
	if (reserveTokens !== undefined && config.contextReserveTokens === undefined)
		config.contextReserveTokens = reserveTokens;
