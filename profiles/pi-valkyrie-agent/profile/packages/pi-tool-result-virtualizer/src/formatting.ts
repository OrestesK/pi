...
function snippetAroundRange(
	...
	if (matchBytes + markerBytes >= maxBytes)
		return `${prefixMarker}${byteSafePrefix(matchText, Math.max(0, maxBytes - markerBytes))}${suffixMarker}`;
...
export function capProtectedToolOutput(
	text: string,
	notice: string,
): {
	text: string;
	...
	const capped =
		suffixBytes >= PROTECTED_TOOL_OUTPUT_BYTE_LIMIT
	...
	return {
		...
		totalBytes,
		returnedBytes: byteLength(capped),
	};
}
...
function formatSearchContext(match: SearchMatch): {
	text: string;
	contextTruncated: boolean;
} {
	if (byteLength(match.context) <= SEARCH_MATCH_CONTEXT_BYTE_LIMIT)
		return { text: match.context, contextTruncated: false };
	return {
		text: [
			snippetAroundRange(
				match.line,
				...
				SEARCH_MATCH_CONTEXT_BYTE_LIMIT,
			),
			`[tool-result-virtualizer] context capped around matching line; use bounded tool_result_get windows for sourceId:"${match.sourceId}" starting at line ${match.contextStartLine}`,
		].join("\n"),
		contextTruncated: true,
	};
}

export function formatSearchMatches(matches: SearchMatch[]): {
	text: string;
	contextTruncated: boolean;
} {
...
export function formatDiagnostics(report: StoreConsistencyReport): string {
	const recent =
		report.recentSources.length === 0
			? ["Recent: none"]
			: [
					"Recent:",
					...report.recentSources.map(
						(source) =>
	...
	const issues =
		report.issues.length === 0
			...
			: [
					...
					...report.issues.map(
						(issue) =>
							`- ${issue.code}: ${issue.count}${issue.sourceIds ? `; samples: ${issue.sourceIds.join(", ")}` : ""}`,
					),
				];
	const maxSources = report.quota.maxSources ?? "disabled";
	const maxStoredBytes = report.quota.maxStoredBytes ?? "disabled";
	return [
		`Store consistency: ${report.healthy ? "healthy" : "issues detected"}; scope=${report.scope}`,
		`Sources: ${report.validSourceCount}; index lines: ${report.indexLineCount}; invalid index lines: ${report.invalidIndexLineCount}`,
		`Footprint bytes (${report.footprint.scope}; unattributed store files excluded when scoped): sources=${report.footprint.sourceBytes} details=${report.footprint.detailsBytes} index=${report.footprint.indexBytes} fts=${report.footprint.ftsBytes} total=${report.footprint.totalBytes}`,
		`Quota usage (${report.quota.usageScope}; limits are store-wide): sources=${report.quota.currentSources}/${maxSources}; stored bytes=${report.quota.currentStoredBytes}/${maxStoredBytes}; exceeded=${report.quota.sourceLimitExceeded || report.quota.storedBytesLimitExceeded}`,
		`FTS: ${report.ftsStatus}; mismatches: ${report.ftsMismatchCount}; scope key unavailable: ${report.scopeKeyUnavailable}`,
		...issues,
		...recent,
	].join("\n");
}

export function formatRetentionPreview(
	preview: RetentionPreview,
	limit: number,
): string {
	const selectorText =
		Object.entries(preview.selectors)
		...
		visibleCandidates.length === 0
			...
			: [
					...
					...visibleCandidates.map(
						(candidate) =>
							`- ${candidate.sourceId} ${candidate.toolName} ${candidate.captureStatus} ${candidate.storageKind} ${candidate.byteCount} bytes ${candidate.lineCount} lines reasons:${candidate.reasons.join(",")}`,
					),
				];
