...
This fork registers these tools (plus `ast_grep_outline`, `ast_grep_dump`, and `lsp_navigation`) active by default. `pi_lens_activate_tools` remains an additive compatibility catalog; activation is not required before a call.
...
Returns the full indented AST with node kinds and positions. Then use the correct kind in your pattern or `insideKind`. `ast_grep_dump` is active by default in this fork.
