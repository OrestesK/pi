...
describe("index.ts LSP idle reset", () => {
	...
	it(
		"subagent session fires the idle reset at 60s, not 240s (#713)",
		async () => {
	...
	it(
		"normal (non-subagent) session still uses 240s idle reset (#713)",
		async () => {
	...
	it(
		"PI_LENS_SUBAGENT_FULL=1 restores 240s idle reset even in a subagent session (#713)",
		async () => {
			process.env.PI_SUBAGENT_CHILD = "1";
