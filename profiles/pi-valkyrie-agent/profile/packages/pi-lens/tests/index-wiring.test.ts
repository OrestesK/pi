...
// Mock heavy real-work seams so lifecycle emissions remain fast,
// deterministic wiring checks rather than real scans/LSP bootstraps.
const lifecycleMocks = vi.hoisted(() => ({
	handleToolResult: vi.fn(),
	flushDebouncedToolResults: vi.fn(),
...
vi.mock("../clients/runtime-tool-result.js", () => ({
	handleToolResult: lifecycleMocks.handleToolResult,
	flushDebouncedToolResults: lifecycleMocks.flushDebouncedToolResults,
	clearLastAnalyzedStateCache: lifecycleMocks.clearLastAnalyzedStateCache,
}));
vi.mock("../clients/runtime-agent-end.js", () => ({
	handleAgentEnd: lifecycleMocks.handleAgentEnd,
}));
vi.mock("../clients/runtime-turn.js", () => ({
	handleTurnEnd: lifecycleMocks.handleTurnEnd,
	cancelLSPIdleReset: lifecycleMocks.cancelLSPIdleReset,
}));
...
describe("index.ts extension wiring", () => {
	describe("registration", () => {
		it("registers every expected flag, command, tool, and lifecycle hook", () => {
			...
			expect(pi.flags.get("no-lens")?.default).toBe(true);
		...
		// The tools-only fork keeps every native tool manually callable after
		// session initialization; only automatic read/write/end pipelines are off.
		it("keeps every registered tool active on a dynamic-tooling host", async () => {
		...
		// and every tool stays statically active, matching this fork's policy.
		it("falls back to all tools statically active on a host without dynamic-tooling support", async () => {
			...
			try {
				...
				for (const t of EXPECTED_TOOLS) {
					expect(pi.getTool(t), `tool registered: ${t}`).toBeDefined();
					// Every registered tool remains active on hosts with or without
					// dynamic-tool APIs.
	...
	describe("tools-only lifecycle gating", () => {
		beforeEach(() => {
			vi.clearAllMocks();
		});

		it("skips automatic tool-result, agent-end, and turn-end work by default", async () => {
			const pi = createPiMock();
			extension(pi.asExtensionAPI());
			const ctx = makeCtx();

			await pi.emit("tool_result", { toolName: "write" }, ctx);
			await pi.emit("agent_end", {}, ctx);
			await pi.emit("turn_end", {}, ctx);

			expect(lifecycleMocks.handleToolResult).not.toHaveBeenCalled();
			expect(lifecycleMocks.handleAgentEnd).not.toHaveBeenCalled();
			expect(lifecycleMocks.handleTurnEnd).not.toHaveBeenCalled();
			expect(lifecycleMocks.flushDebouncedToolResults).not.toHaveBeenCalled();
		});

		it("runs automatic lifecycle work after explicit /lens-toggle opt-in", async () => {
			const pi = createPiMock();
			extension(pi.asExtensionAPI());
			const ctx = makeCtx();
			...
			expect(lifecycleMocks.handleToolResult).toHaveBeenCalledOnce();
			expect(lifecycleMocks.handleAgentEnd).toHaveBeenCalledOnce();
			expect(lifecycleMocks.handleTurnEnd).toHaveBeenCalledOnce();
			expect(lifecycleMocks.flushDebouncedToolResults).toHaveBeenCalledTimes(2);
		});
	});
	...
	describe("context injection gating + toggle", () => {
		...
		it("suppresses injection when --no-lens-context is set, then injects after /lens-context-toggle", async () => {
			// Keep the full lens explicitly enabled while isolating the context flag.
			const pi = createPiMock({
				"no-lens": false,
				"no-lens-context": true,
			});
			extension(pi.asExtensionAPI());
