...
> unreviewed install scripts, review and allow only the required scripts with
> `npm approve-scripts`; this package does not declare a package-level
> `allowScripts` policy. Installing from a git source (`pi install git:...` /
> `pi update --extension git:...`) may similarly prompt for git-dependency
> approval; accept it to let the `prepare` build step run.
