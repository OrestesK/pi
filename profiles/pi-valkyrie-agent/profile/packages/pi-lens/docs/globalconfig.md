...
`contextInjection.enabled` (default `true`) controls whether pi-lens prepends automatic findings — session-start guidance, turn-end findings, and test findings — into the next model turn after `/lens-toggle` enables automatic processing. Set it to `false` (or use `--no-lens-context` / `PI_LENS_NO_CONTEXT_INJECTION=1` / `/lens-context-toggle`) to keep native tools available and, after opt-in, retain other automatic pipelines without context injection. Findings are still cached, so `lens_diagnostics` and `/lens-health` keep working.
