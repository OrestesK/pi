...
**Tool availability.** This fork registers every native tool active, including
`ast_grep_search`, `ast_grep_replace`, `ast_grep_outline`, `ast_grep_dump`,
`lsp_navigation`, and `lens_diagnostic_mark`. The
`pi_lens_activate_tools` catalog remains additive and idempotent for prompt
compatibility; callers do not need it before using another tool
(`tools/activate-tools.ts`, wired in `index.ts`).
