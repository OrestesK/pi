...
This fork always runs `session_start` initialization: it resets runtime state,
hydrates project caches, preinstalls likely tools, warms LSPs, and starts
background project scans. Native agent tools remain manually callable.

Automatic lifecycle work starts disabled. After `/lens-toggle` opts in for the
current session:
...
After `/lens-toggle` enables automatic processing, each write/edit runs a
...
```bash
# Tools and session initialization enabled; automatic pipelines disabled
pi

# Enable automatic processing for the current session
/lens-toggle

# Optional switches
pi --no-lens             # Explicitly retain the default tools-only mode
pi --no-lens-context     # After opt-in, disable context injection only; /lens-context-toggle
...
Session-level scanners run in the background and remain queryable through
manual tools. They surface automatically at turn end only after `/lens-toggle`:
