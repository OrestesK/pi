...
async function extractEntitySnapshot(client, filePath, languageId) {
...
const treeSitterRunner = {
    ...
    async run(ctx) {
        ...
        const builtinRulesDir = resolvePackagePath(import.meta.url, "rules", "tree-sitter-queries", languageId);
        const ruleFileSet = new Set();
        for (const dir of [rulesDir, builtinRulesDir]) {
            if (fs.existsSync(dir)) {
                for (const f of fs.readdirSync(dir)) {
                    if (f.endsWith(".yml"))
                        ruleFileSet.add(path.join(dir, f));
                }
            }
        }
        ...
        for (let i = 0; i < effectiveQueries.length; i += CONCURRENCY_LIMIT) {
            ...
            const batchResults = await Promise.all(batch.map(async (query) => {
                ...
                try {
                    const matches = await client.runQueryOnFile(query, filePath, languageId, { maxResults: 10 }, contentOverride);
                    for (const match of matches) {
        ...
        const diagnostics = queryResults.flat();
        ...
        const skipEntityExtraction = totalLinesChanged < ENTITY_EXTRACTION_LINE_THRESHOLD;
