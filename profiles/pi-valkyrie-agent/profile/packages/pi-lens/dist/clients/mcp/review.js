 ...
 * stray tsconfig.
 */
export function canRebuildPiLens(repoRoot) {
    const inNodeModules = /(^|[\\/])node_modules([\\/]|$)/i.test(repoRoot);
    return !inNodeModules && existsSync(join(repoRoot, "tsconfig.dist.json"));
}
export const REBUILD_UNAVAILABLE_MESSAGE = "pilens_rebuild is unavailable in an installed pi-lens package. Rebuilding is only safe from a source checkout containing tsconfig.dist.json; reinstall or update pi-lens through your package manager instead.";
/**
