...
export async function tryExpandRead(filePath, requestedOffset, requestedLimit, totalLines, tsClient) {
    ...
    try {
        ...
        const tree = await withBudget(tsClient.parseFile(filePath, languageId, content), remaining);
        if (!tree)
            return undefined;
        ...
        const enclosing = findEnclosingNodeForRange(tree.rootNode, requestedStartRow, requestedEndRow, enclosingTypes);
        if (!enclosing)
            return undefined;
        const symbolStart = enclosing.startPosition.row + 1;
        // biome-ignore lint/suspicious/noExplicitAny: endPosition not in local interface
        const symbolEnd = (enclosing.endPosition?.row ?? enclosing.startPosition.row) + 1;
