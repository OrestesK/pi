...
let _startupScanVerdictTtlCache;
/**
 ...
 */
export function getStartupScanVerdictTtlMs() {
    if (_startupScanVerdictTtlCache !== undefined) {
        return _startupScanVerdictTtlCache;
    }
    const envTtl = toPositiveFinite(process.env.PI_LENS_STARTUP_SCAN_VERDICT_TTL_MS);
...
export function _resetStartupScanVerdictTtlForTests() {
...
function initSourceCountWalk(dir) {
 ...
 */
function classifyCountEntry(entry, fullPath, ignoreMatcher, stack) {
...
export async function resolveStartupScanContextAsync(cwd, options = {}) {
    ...
    if (!projectRoot) {
        ...
        const sourceFileCount = await countSourceFilesWithinLimitAsync(projectRoot, maxSourceFiles);
        if (sourceFileCount > maxSourceFiles) {
            result = {
                cwd: resolvedCwd,
                scanRoot: projectRoot,
