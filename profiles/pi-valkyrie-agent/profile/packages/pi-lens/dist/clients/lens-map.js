 ...
 */
export function aggregateGraphToFiles(graph, options) {
    ...
    function isTestFile(id, filePath) {
    ...
    const droppedIgnoredIds = new Set();
    if (excludeIds) {
        for (const id of allFileIds) {
    ...
    function buildEdgeList(keepIds) {
        const out = [];
        for (const [key, weight] of edgeWeights) {
            const sep = key.indexOf("\u0000");
            const from = key.slice(0, sep);
            const to = key.slice(sep + 1);
            if (keepIds && (!keepIds.has(from) || !keepIds.has(to)))
                continue;
    ...
    const keptEntries = [...files.values()].filter((entry) => !keepIds || keepIds.has(entry.id));
    const nodes = keptEntries
        .map((entry) => ({
        id: entry.id,
        path: entry.path,
 ...
 */
export function renderMapHtml(payload) {
 ...
 */
export async function generateLensMap(cwd) {
