...
export function getManagedLogFiles(dir = LOG_DIR) {
    const names = new Set(UNMANAGED_STRAGGLER_LOG_FILES);
...
function getConfig() {
    return {
        ...
        maxSizeMB: Math.max(1, Number.parseInt(process.env.PI_LENS_MAX_LOG_SIZE_MB ?? "10", 10) || 10),
    };
}
 ...
 */
export function cleanupOldLogs(directory, pattern, retentionDays) {
 ...
 */
export function rotateLogIfNeeded(logFile, maxSizeMB) {
    const config = getConfig();
 ...
 */
export function runLogCleanup(dbg) {
    const config = getConfig();
    const results = {
        cleaned: 0,
        rotated: 0,
    ...
    for (const logFile of mainLogs) {
        ...
        if (rotation.rotated) {
            ...
            if (rotation.newFile) {
                ...
                dbg?.(`log_cleanup: rotated ${path.basename(logFile)} (${sizeMB.toFixed(1)}MB) → ${path.basename(rotation.newFile)}`);
            }
        }
    }
    // Build report
