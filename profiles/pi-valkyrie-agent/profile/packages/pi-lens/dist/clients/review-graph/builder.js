...
import { isAtOrAboveHomeDir, normalizeFilePath, normalizeMapKey, } from "../path-utils.js";
...
export function clearReviewGraphWorkspaceCache() {
    _buildCache.clear();
 ...
 */
async function sourceHashMapAsync(files) {
...
function getReviewGraphMaxFiles() {
...
function writePending(key) {
    ...
    try {
        ...
        console.error("[review-graph] cache serialize failed:", err.message);
        return;
    }
    logLatency({
        type: "phase",
...
async function ensureReviewGraphFacts(filePath, cwd, facts) {
...
async function extractTreeSitterSymbols(filePath, languageId) {
    ...
    for (const imp of extracted.imports) {
        ...
        if (resolved.length > 0) {
            for (const target of resolved) {
                const toNode = ensureFileNode(graph, target, mapKindToTreeSitterLanguage(detectFileKind(target), target) ??
                    languageId);
                addEdge(graph, {
                    from: fileNodeId,
                    to: toNode,
...
function ensureFileNode(graph, filePath, languageId) {
    const normalized = normalizeMapKey(filePath);
...
function addCxxIncludeEdges(graph, cwd, filePath, ignoredIds) {
    let content = "";
    try {
...
async function addFileToGraph(graph, cwd, file, facts, ignoredIds) {
    const kind = detectFileKind(file);
    ...
    if (kind === "cxx")
        addCxxIncludeEdges(graph, cwd, file, ignoredIds);
}
...
function resolveDeferredSymbolEdges(graph) {
...
async function tryIncrementalFromCache(cached, ctx) {
    ...
    persistGraph(ctx.cwd, ctx.signature, ctx.fileSignatures, hashes, graphSnapshot);
