...
import { analyzeFile, analyzeFileFresh, createMcpHost, diagnosticStats, ensureLspConfig, ipcPathForCwd, lspStatus, moduleReport, projectScan, readEnclosing, readSymbol, recentLatency, renderCompactModuleReport, resolveRebuildScript, resourceFootprint, runRebuild, runSessionStart, runTurnEnd, summarizeScan, symbolSearch, } from "../clients/lens-engine.js";
...
async function callTool(name, args) {
    ...
    if (name === "pilens_symbol_search") {
        ...
        const limit = typeof args.limit === "number" && Number.isFinite(args.limit)
            ...
            : 20;
        const { available, results, hint, snapshotGeneratedAt } = symbolSearch(query, cwd, limit);
        if (!available) {
            return toolText(hint ?? "No word index for this workspace yet — run pilens_session_start first.", { available: false, query, hint }, true);
        }
        const stalenessNote = graphStalenessNote(snapshotGeneratedAt, "Project snapshot");
        if (results.length === 0) {
            return toolText(`No files matched "${query}".` + (stalenessNote ? `\n\n${stalenessNote}` : ""), {
                available: true,
                query,
                results: [],
