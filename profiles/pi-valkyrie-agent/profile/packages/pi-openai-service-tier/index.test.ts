...
test("resolves global config from PI_CODING_AGENT_DIR", () => {
  const cwd = tempDir();
  const home = tempDir();
  const agentDir = join(tempDir(), "pi-config");
  try {
    const paths = configPaths(cwd, home, { PI_CODING_AGENT_DIR: agentDir });
    assert.equal(paths.global, join(agentDir, "extensions", "pi-openai-service-tier.json"));
  } finally {
    rmSync(cwd, { recursive: true, force: true });
    rmSync(home, { recursive: true, force: true });
...
test("resolves default and tilde agent directories", () => {
  const home = tempDir();
  try {
    assert.equal(resolveAgentDir(home, {}), join(home, ".pi", "agent"));
    assert.equal(resolveAgentDir(home, { PI_CODING_AGENT_DIR: "  " }), join(home, ".pi", "agent"));
    assert.equal(resolveAgentDir(home, { PI_CODING_AGENT_DIR: "~" }), home);
    assert.equal(resolveAgentDir(home, { PI_CODING_AGENT_DIR: "~/pi-config" }), join(home, "pi-config"));
  } finally {
    rmSync(home, { recursive: true, force: true });
  }
});
...
test("applies configured tier only when active, allow-listed, and tier-supported", () => {
  ...
  for (const id of ["gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"]) {
    assert.equal(
      supportsServiceTier({ provider: "openai-codex", id, api: "openai-codex-responses" } as never, supportedModels),
      true,
      `${id} should support Priority processing`,
    );
  }
  assert.equal(
