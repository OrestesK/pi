...
import {
	...
	SKILLS_ROOT_ENV,
	projectResourcesIgnored,
} from "../../shared/project-resources.ts";
...
export function buildPiArgs(input: BuildPiArgsInput): BuildPiArgsResult {
	...
	const ignoreProjectResources = projectResourcesIgnored();
	...
	if (ignoreProjectResources) {
		...
		const configuredExtensions = (process.env[RUNTIME_EXTENSIONS_ENV] ?? "")
			.split(path.delimiter)
			.map((value) => value.trim())
			.filter(Boolean);
		const mcpExtension = declaredBuiltinTools.includes("mcp") || input.mcpDirectTools?.length
			? process.env[MCP_EXTENSION_ENV]
			: undefined;
		...
		if (input.inheritSkills && process.env[SKILLS_ROOT_ENV]) {
			args.push("--skill", process.env[SKILLS_ROOT_ENV]!);
		}
	} else if (input.extensions !== undefined) {
	...
	if (!ignoreProjectResources && !input.inheritSkills) {
		args.push("--no-skills");
	}
	...
	env[SUBAGENT_PARENT_CAPABILITY_TOKEN_ENV] = fanoutAuthorized
		...
		: "";
	env[PROJECT_RESOURCES_ENV] = ignoreProjectResources ? "ignore" : "inherit";
	env.PI_SUBAGENT_INHERIT_PROJECT_CONTEXT = ignoreProjectResources ? "0" : input.inheritProjectContext ? "1" : "0";
