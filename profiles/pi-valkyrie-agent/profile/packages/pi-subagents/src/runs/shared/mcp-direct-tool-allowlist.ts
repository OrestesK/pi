...
function loadMcpConfig(cwd: string): McpConfig {
	...
	for (const sourcePath of getConfigPaths(cwd)) {
		...
		config = mergeConfigs(
			config,
			projectResourcesIgnored() ? { ...loaded, imports: undefined } : expandImports(loaded, cwd),
		);
	}
	return config;
}
...
function getConfigPaths(cwd: string): string[] {
	if (projectResourcesIgnored()) {
		const runtimeConfig = process.env[PROFILE_MCP_CONFIG_ENV];
		return runtimeConfig ? [path.resolve(runtimeConfig)] : [];
	}
