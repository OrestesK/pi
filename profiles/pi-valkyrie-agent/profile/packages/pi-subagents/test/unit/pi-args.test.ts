...
const originalEnv = {
	...
	[PROJECT_RESOURCES_ENV]: process.env[PROJECT_RESOURCES_ENV],
	[RUNTIME_EXTENSIONS_ENV]: process.env[RUNTIME_EXTENSIONS_ENV],
	[MCP_EXTENSION_ENV]: process.env[MCP_EXTENSION_ENV],
	[PROFILE_MCP_CONFIG_ENV]: process.env[PROFILE_MCP_CONFIG_ENV],
	[SKILLS_ROOT_ENV]: process.env[SKILLS_ROOT_ENV],
};
...
describe("buildPiArgs system prompt mode wiring", () => {
	...
	it("enforces closed child resources while preserving fanout and supervisor wiring", () => {
		process.env[PROJECT_RESOURCES_ENV] = "ignore";
		process.env[RUNTIME_EXTENSIONS_ENV] = ["/trusted/lens.ts", "/trusted/fff.ts"].join(path.delimiter);
		process.env[MCP_EXTENSION_ENV] = "/trusted/mcp.ts";
		...
		const { args, env } = buildPiArgs({
			...
			inheritSkills: true,
			tools: ["read", "mcp", "subagent"],
			extensions: ["/workspace/malicious.ts"],
			orchestratorIntercomTarget: "parent",
			parentSessionId: "session-1",
			runId: "run-1",
			childAgentName: "clone",
			childIndex: 0,
		});
		...
		assert.ok(extensionArgs.includes("/trusted/lens.ts"));
		assert.ok(extensionArgs.includes("/trusted/fff.ts"));
		assert.ok(extensionArgs.includes("/trusted/mcp.ts"));
		assert.equal(extensionArgs.includes("/workspace/malicious.ts"), false);
		assert.ok(extensionArgs.some((value) => value.endsWith("fanout-child.ts")));
		assert.equal(args[args.indexOf("--skill") + 1], "/trusted/skills");
		assert.equal(env[PROJECT_RESOURCES_ENV], "ignore");
		assert.equal(env.PI_SUBAGENT_INHERIT_PROJECT_CONTEXT, "0");
	...
	it("resolves direct MCP tools only from the runtime config in closed mode", () => {
		const fixture = createMcpFixture();
		writeMcpFixture(fixture, {
			serverName: "trusted",
			definition: { command: "trusted-server" },
			tools: [{ name: "safe_operation" }],
		});
		writeJson(path.join(fixture.root, "home", ".config", "mcp", "mcp.json"), {
			mcpServers: { trusted: { command: "malicious-global-server" } },
			imports: ["vscode"],
		});
		writeJson(path.join(fixture.projectDir, ".mcp.json"), {
			mcpServers: { trusted: { command: "malicious-project-server" } },
		});
		writeJson(path.join(fixture.projectDir, ".pi", "mcp.json"), {
			mcpServers: { trusted: { command: "malicious-project-pi-server" } },
			imports: ["vscode"],
		});
		writeJson(path.join(fixture.projectDir, ".vscode", "mcp.json"), {
			servers: { trusted: { command: "malicious-imported-server" } },
		});
		process.env[PROJECT_RESOURCES_ENV] = "ignore";
		process.env[PROFILE_MCP_CONFIG_ENV] = path.join(fixture.agentDir, "mcp.json");
		...
		const { args } = buildPiArgs({
			...
			inheritSkills: false,
			tools: ["read", "mcp"],
			mcpDirectTools: ["trusted"],
			cwd: fixture.projectDir,
		});

		assert.equal(args[args.indexOf("--tools") + 1], "read,mcp,trusted_safe_operation");
	});
