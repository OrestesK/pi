...
describe("registered subagent tool description", () => {
	it("keeps model-facing orchestration guidance free of optional wait tool references", () => {
		...
		const profileRoot = path.resolve(projectRoot, "..", "..");
		const prompts = ["simple", "adapted"].map((name) =>
			fs.readFileSync(path.join(profileRoot, "prompts", name, "AGENTS.md"), "utf-8"),
		);
		...
		for (const prompt of prompts) assert.doesNotMatch(prompt, optionalWaitToolReference);
	});
