...
describe("resolveIntercomBridge", () => {
	...
	it("keeps the native supervisor target without activating the public bridge", () => {
		const bridge = resolveIntercomBridge({
			config: { mode: "off" },
			context: "fresh",
			orchestratorTarget: "main",
		...
		assert.equal(bridge.orchestratorTarget, "main");
		const unchanged = applyIntercomBridgeToAgent(makeAgent({ tools: ["read"] }), bridge);
		assert.deepEqual(unchanged.tools, ["read"]);
		assert.equal(unchanged.systemPrompt, "Base prompt");
	});
