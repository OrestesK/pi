...
import {
	...
	RUNTIME_EXTENSIONS_ENV,
	SKILLS_ROOT_ENV,
} from "../../src/shared/project-resources.ts";
...
describe("single sync execution", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("keeps the closed spawn contract for foreground single, parallel, and chain routes", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		const previous = {
			project: process.env[PROJECT_RESOURCES_ENV],
			extensions: process.env[RUNTIME_EXTENSIONS_ENV],
			skills: process.env[SKILLS_ROOT_ENV],
		};
		process.env[PROJECT_RESOURCES_ENV] = "ignore";
		process.env[RUNTIME_EXTENSIONS_ENV] = "/trusted/runtime-extension.ts";
		process.env[SKILLS_ROOT_ENV] = "/trusted/skills";
		try {
			const clone = makeAgent("clone", { tools: ["read", "subagent"], inheritSkills: true });
			const executor = makeExecutor([clone], { intercomBridge: { mode: "off" } });
			for (const response of ["single", "parallel-a", "parallel-b", "chain-a", "chain-b"]) {
				mockPi.onCall({ output: response });
			}
			const signal = new AbortController().signal;
			const ctx = makeMinimalCtx(tempDir);
			await executor.execute("closed-single", { agent: "clone", task: "single" }, signal, undefined, ctx);
			await executor.execute("closed-parallel", {
				tasks: [
					{ agent: "clone", task: "parallel a" },
					{ agent: "clone", task: "parallel b" },
				],
			}, signal, undefined, ctx);
			await executor.execute("closed-chain", {
				chain: [
					{ agent: "clone", task: "chain a" },
					{ agent: "clone", task: "chain b" },
				],
			}, signal, undefined, ctx);
			...
			for (const call of calls) {
				...
				assert.equal(call.env?.PI_SUBAGENT_PROJECT_RESOURCES, "ignore");
				assert.equal(call.env?.PI_SUBAGENT_INHERIT_PROJECT_CONTEXT, "0");
				assert.equal(call.env?.PI_SUBAGENT_FANOUT_CHILD, "1");
				assert.equal(call.env?.PI_SUBAGENT_ORCHESTRATOR_TARGET?.startsWith("subagent-chat-"), true);
				assert.equal(call.env?.PI_SUBAGENT_ORCHESTRATOR_SESSION_ID !== null, true);
				assert.match(call.env?.PI_SUBAGENT_SUPERVISOR_CHANNEL_DIR ?? "", /supervisor-channels/);
				const args = call.args ?? [];
				for (const flag of ["--no-context-files", "--no-extensions", "--no-skills", "--no-prompt-templates", "--no-themes"]) assert.equal(args.includes(flag), true, flag);
				assert.equal(args[args.indexOf("--tools") + 1].split(",").includes("intercom"), false);
				assert.equal(args.filter((_arg, index) => args[index - 1] === "--extension").includes("/trusted/runtime-extension.ts"), true);
				assert.equal(args[args.indexOf("--skill") + 1], "/trusted/skills");
				if (call.env?.PI_SUBAGENT_SUPERVISOR_CHANNEL_DIR) fs.rmSync(call.env.PI_SUBAGENT_SUPERVISOR_CHANNEL_DIR, { recursive: true, force: true });
			}
		} finally {
			for (const [name, value] of Object.entries({
				[PROJECT_RESOURCES_ENV]: previous.project,
				[RUNTIME_EXTENSIONS_ENV]: previous.extensions,
				[SKILLS_ROOT_ENV]: previous.skills,
			})) {
