...
export function loadSessionMcpConfig(
  pi: Pick<ExtensionAPI, "getFlag">,
  cwd: string,
): McpConfig {
  const configPath = pi.getFlag("mcp-config") as string | undefined;
  const ignoreProjectConfigs = pi.getFlag("mcp-ignore-project-config") === true;
  return loadMcpConfig(configPath, cwd, { ignoreProjectConfigs });
}
...
export async function initializeMcp(
  ...
  const config = loadSessionMcpConfig(pi, ctx.cwd);
