...
import { getConfigPathFromArgv, getIgnoreProjectConfigFromArgv, normalizeDirectToolInputSchema, truncateAtWord } from "./utils.ts";
import { initializeOAuth, shutdownOAuth } from "./mcp-auth-flow.ts";
import { createMcpDirectToolCallRenderer, renderMcpProxyToolCall, renderMcpToolResult } from "./tool-result-renderer.ts";
import { toolErrorOverride } from "./error-signal.ts";

export function loadEarlyMcpConfig(cwd = process.cwd()) {
  const configPath = getConfigPathFromArgv();
  const configLoadOptions = {
    ignoreProjectConfigs: getIgnoreProjectConfigFromArgv(),
  };
  return {
    configPath,
    configLoadOptions,
    config: loadMcpConfig(configPath, cwd, configLoadOptions),
  };
}
...
export default function mcpAdapter(pi: ExtensionAPI) {
  ...
  pi.registerFlag("mcp-ignore-project-config", {
    description: "Ignore project and imported ambient MCP configuration",
    type: "boolean",
    default: false,
  });
    ...
    handler: async (args, ctx) => {
      ...
      switch (subcommand) {
        ...
        case "tools":
          ...
          break;
        case "setup": {
          const result = await openMcpSetup(state, pi, ctx, earlyConfigPath, "setup", configLoadOptions);
        ...
        default:
          if (ctx.hasUI) {
            const result = await openMcpPanel(state, pi, ctx, earlyConfigPath, configLoadOptions);
            if (result?.configChanged) {
              await ctx.reload();
              return;
            }
  ...
  pi.registerCommand("mcp-auth", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!serverName) {
        await openMcpAuthPanel(state, pi, ctx, earlyConfigPath, configLoadOptions);
        return;
      }
