...
import type { McpConfigLoadOptions } from "./config.ts";
import {
...
function buildSharedConfigNoticeLines(
  ...
  const discovery = getMcpDiscoverySummary(configOverridePath, cwd, options);
...
export async function openMcpSetup(
  ...
  const callbacks = {
    ...
    scaffoldProjectConfig: async () => {
      if (options.ignoreProjectConfigs) throw new Error("Project MCP configuration is disabled");
...
export async function openMcpPanel(
  ...
  if (Object.keys(state.config.mcpServers).length === 0) {
    return openMcpSetup(state, pi, ctx, configOverridePath, "empty", options);
  }

  const config = state.config;
  const cache = loadMetadataCache();
  const configPath = pi.getFlag("mcp-config") as string | undefined ?? configOverridePath;
  const provenanceMap = getServerProvenance(configPath, ctx.cwd, options);
  const { lines: noticeLines, fingerprint } = buildSharedConfigNoticeLines(configPath, ctx.cwd, options);
