...
export function getConfigDiscoveryPaths(
  ...
  return getConfigSources(overridePath, cwd, options).map((source) => ({
    label: source.label,
...
export function findAvailableImportConfigs(
  cwd = process.cwd(),
  options: McpConfigLoadOptions = {},
): DiscoveredImportConfig[] {
  if (options.ignoreProjectConfigs) return [];
...
export function getMcpDiscoverySummary(
  ...
  const sources = getConfigSources(overridePath, cwd, options).map((source) => {
  ...
  const imports = options.ignoreProjectConfigs ? [] : (Object.keys(IMPORT_PATHS) as ImportKind[])
    .map((kind) => {
...
export function loadMcpConfig(
  ...
  for (const source of getConfigSources(overridePath, cwd, options)) {
    const loaded = readValidatedConfig(source.readPath, `MCP config from ${source.readPath}`);
    if (!loaded) continue;
    config = mergeConfigs(config, options.ignoreProjectConfigs ? { ...loaded, imports: undefined } : expandImports(loaded, cwd));
  }
...
function getConfigSources(
  ...
  if (options.ignoreProjectConfigs) {
    return overridePath
      ? [{
          id: "pi-global",
...
export function getServerProvenance(
  ...
  for (const source of getConfigSources(overridePath, cwd, options)) {
    ...
    if (!options.ignoreProjectConfigs && loaded.imports?.length) {
