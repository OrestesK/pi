...
TRANSPORT_EXCLUDED_DIRECTORY_NAMES = frozenset({"__pycache__", ".git", ".venv", "venv"})
TRANSPORT_EXCLUDED_FILE_NAMES = frozenset({".env", ".DS_Store"})
TRANSPORT_EXCLUDED_FILE_SUFFIXES = (".pyc", ".pyo", ".pyd", ".so", ".dll", ".dylib")
...
HASH_EXCLUDED_DIRECTORY_NAMES = TRANSPORT_EXCLUDED_DIRECTORY_NAMES | GENERATED_DIRECTORY_NAMES
...
def hash_directory(path: Path) -> str:
    ...
    for candidate in sorted(path.rglob("*")):
        ...
        if any(
            part in HASH_EXCLUDED_DIRECTORY_NAMES or part.endswith(".egg-info")
        ...
        if (
            ...
            or candidate.name in TRANSPORT_EXCLUDED_FILE_NAMES
            or candidate.name.endswith(TRANSPORT_EXCLUDED_FILE_SUFFIXES)
        ):
            continue
