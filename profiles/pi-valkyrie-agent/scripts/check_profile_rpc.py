...
def main() -> int:
    ...
    try:
        ...
        mcp_config = runtime / "mcp.runtime.json"
        write_json_atomic(mcp_config, build_runtime_mcp_config(profile, None, root))
        ...
        project_config = workspace / ".pi"
        project_extensions = project_config / "extensions"
        project_extensions.mkdir(parents=True)
        (workspace / ".vscode").mkdir()
        (workspace / "AGENTS.md").write_text("UNTRUSTED_CONTEXT_MARKER\n")
        (project_config / "settings.json").write_text(
            json.dumps({"defaultProvider": "google", "defaultModel": "malicious"})
        )
        (project_extensions / "malicious.ts").write_text(
            'export default function (pi) { pi.registerTool({ name: "malicious_project_tool", parameters: {}, execute: async () => ({ content: [] }) }); }\n'
        )
        malicious_mcp = json.dumps(
            {"mcpServers": {"malicious-project": {"url": "http://project.invalid/mcp"}}}
        )
        (workspace / ".mcp.json").write_text(malicious_mcp)
        (project_config / "mcp.json").write_text(malicious_mcp)
        (workspace / ".vscode" / "mcp.json").write_text(malicious_mcp)
        completed = subprocess.run(
            build_pi_argv(
                paths,
                allowed_tools,
                select_prompt_profile(profile, "simple"),
                mcp_config,
            ),
            cwd=workspace,
            env=env,
