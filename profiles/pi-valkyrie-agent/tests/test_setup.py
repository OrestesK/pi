...
def test_setup_uses_only_pinned_bundle_local_runtime() -> None:
    ...
    assert 'npm ci --prefix "$PROFILE" --omit=dev --ignore-scripts' in script
    assert 'SETUP_HOME="$PROFILE/runtime/setup-home"' in script
    assert 'export HOME="$SETUP_HOME"' in script
    assert 'export XDG_CACHE_HOME="$PROFILE/runtime/xdg-cache"' in script
    assert 'export XDG_CONFIG_HOME="$PROFILE/runtime/xdg-config"' in script
    assert 'export XDG_STATE_HOME="$PROFILE/runtime/xdg-state"' in script
    assert 'export npm_config_cache="$NPM_CACHE"' in script
    assert 'export npm_config_logs_dir="$NPM_LOGS"' in script
    assert "export npm_config_ignore_scripts=true" in script
    assert '"$NODE" "$PROFILE/node_modules/@ast-grep/cli/postinstall.js"' in script
    assert '"$NODE" "$PROFILE/node_modules/tsx/node_modules/esbuild/install.js"' in script
