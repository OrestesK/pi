...
description: |-
  Evaluate comparative agent, workflow, prompt, or skill-activation behavior. Use for baseline/candidate task matrices, fresh-context activation probes, outcome judging, and promotion evidence. Do not use to author skill structure, prove product behavior, or change Pi configuration
  use skill-authoring, behavioral-proof, or runtime-maintenance
---

# Agent Evaluation

Evaluate observable outcomes under controlled conditions. An evaluation produces evidence and a recommendation:
- it does not promote a candidate or modify runtime configuration
...
A recommendation is not promotion authority. Skill-content changes return to `skill-authoring`:
- placement, precedence, wiring, and runtime mutation return to `runtime-maintenance`
